...
(0,custom_select__WEBPACK_IMPORTED_MODULE_0__["default"])('#reportSelect');
...
if (customTabs) {
  ...
  var contentsWrapper = document.querySelector('.custom-tabs__content'); // modal buttons

  var topUpButton = document.querySelector('.no-money__button');
  contentsWrapper.style.height = contents[0].offsetHeight + 'px'; // tab buttons listeners
...
function inputValidate(label, inputName, onErrors, submitBtn, input, form) {
  ...
  onErrors["".concat(inputName)].errorNode.textContent = '';
  ...
  if (label.matches('.custom-input--password')) {
    ...
    if (input.value.length < 6) {
      onErrors["".concat(inputName)].errorNode.textContent = 'Пароль должен содержать минимум 6 символов';
    ...
    if (cyrillicRegex.test(input.value)) {
      onErrors["".concat(inputName)].errorNode.textContent = 'Пароль не должен содержать кириллицы';
      label.classList.add('custom-input--error');
      onErrors["".concat(inputName)].errorFlag = true;
      disabledBtn(Object.values(onErrors).map(function (el) {
        return el.errorFlag;
    ...
    if (reapedPassword) {
      var labelNew = newPassword.closest('label');
  ...
  if (label.matches('.custom-input--mail')) {
    ...
    if (!emailRegex.test(input.value)) {
      onErrors["".concat(inputName)].errorNode.textContent = 'Не валидный Email';
      label.classList.add('custom-input--error');
      onErrors["".concat(inputName)].errorFlag = true;
      disabledBtn(Object.values(onErrors).map(function (el) {
        return el.errorFlag;
  ...
  onErrors["".concat(inputName)].errorFlag = false;
  onErrors["".concat(inputName)].errorNode.textContent = '';
